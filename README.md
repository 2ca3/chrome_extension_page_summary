...
### ポップアップサイズ切替機能
- ヘッダー右上の **S / M / L** ボタンでポップアップサイズを切替
  - S: 400×400px（コンパクト）
  - M: 600×600px（デフォルト）
  - L: 800px × 画面高さ80%（大画面）
- 選択したサイズは次回起動時も維持
...
### v1.2.0 (最新)
- ✨ **ポップアップサイズ切替ボタン（S/M/L）を追加**: サイズは次回起動時も維持
- 🌐 英語サイトでも常に日本語で出力するようプロンプトを改善
- 🔧 Anthropic API の CORS ヘッダーを追加（`anthropic-dangerous-direct-browser-access`）
- ⚙️ 各プロバイダーのモデル選択肢を最新に更新（廃止モデルを削除）
